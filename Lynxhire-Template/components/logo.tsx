...
/** Inline LynxHire logo — amber lynx icon + wordmark. Use variant="dark" on dark backgrounds. */
export function LynxHireLogo({
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <div className="w-8 h-8 rounded-lg bg-[#FF9F1C] flex items-center justify-center">
        <span className="text-white font-black text-sm">L</span>
      </div>
      <span
        className={cn(
          "font-black text-xl tracking-tight",
          variant === "dark" ? "text-white" : "text-[#0B0C0E]"
        )}
      >
        <span className="text-[#FF9F1C]">LYNX</span>HIRE
      </span>
    </div>
  );
}
